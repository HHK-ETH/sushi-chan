...
contract SushiChan is ERC721Enumerable, VRFConsumerBase, Ownable {
    ...
    constructor(address _vrfCoordinator, address _linkToken, string memory _prefixURI)
    ...
    */
    function random(uint256 _tokenId, string memory _attr) public view returns (uint256) {
        require(_exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");

        uint256 randomUint = uint256(keccak256(abi.encode(randomNumber, _tokenId, _attr)));
